...
public class TestServlet5 extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (command.equals("/main.test5")) {
			...
			//    ReqeustDispatcher 객체를 리턴받기(파라미터: 포워딩할 주소(파일명))
			//
			RequestDispatcher dispatcher = req.getRequestDispatcher("jsp09_servlet/test5_main.jsp");
			
			// 2) 1번에서 리번받은 RequestDispatcher 객체의 forward() 메서드 호출하여 포워딩 수행
			//   => 파라미터: HttpServletRequest 객체와 HttpServletResponse 객체
			dispatcher.forward(req, resp);
			
		} else if (command.equals("/list.test5")) {
			// test5_list.jsp를 보여줌
			
			// DB로 부터 목록 조회하는 작업 완료했다고 가정한다면
			// 해당 조회 결과를 request 객체에 저장한 후
			// JSP 페이지를 통해 응답 데이터를 사용자에게 전송하여 출력해야한다!
